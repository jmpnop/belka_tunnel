...
- **Startup update check** — `updater.rs` polls `RELEASES_URL` (`https://api.github.com/repos/jmpnop/belka_tunnel/releases/latest`) once on launch, compares the latest tag to the compiled-in version, and on a newer release fires a notification + opens the release page. (Was previously pointed at the deleted `belka-tunnel` **hyphen** repo — fixed to the underscore repo so in-app update notifications actually work.)
